...
function SandStorm({ active, onDone }) {
...
const startX = -80 + progress * W * 0.6;
const startY = H * 0.25 + Math.random() * H * 0.5;
const targetX = startX + W * 0.5 + Math.random() * W * 0.6;
const targetY = startY + (Math.random() - 0.4) * H * 0.3;
const size = Math.random() * 2.5 + 0.4;
const speed = Math.random() * 0.5 + 0.5;
const delay = progress * 1.0 + Math.random() * 0.2;
const shade = Math.random();
const color = shade > 0.7 ? "#B8922E" : shade > 0.4 ? "#C9A84C" : "#D4A843";
const wobble = (Math.random() - 0.5) * 6;
return { startX, startY, targetX, targetY, size, speed, delay, color, wobble };
});
...
const y = p.startY + (p.targetY - p.startY) * eased + Math.sin(elapsed * 10 + p.wobble) * 3;
...
ctx.globalAlpha = 1;
if (allDone && elapsed > 2.2) { cancelAnimationFrame(animRef.current); onDone && onDone(); return; }
...
useEffect(() => {
const t0 = setTimeout(() => setPhase("show"), 200);
const t1 = setTimeout(() => { setPhase("dissolve"); setStormActive(true); }, 700);
return () => { clearTimeout(t0); clearTimeout(t1); };
}, []);

const handleStormDone = () => { setFadeOut(true); setTimeout(() => onComplete(), 800); };

return (
<div style={{ position: "fixed", inset: 0, zIndex: 1000, background: "#0A0A0A", opacity: fadeOut ? 0 : 1, transition: fadeOut ? "opacity 0.7s ease" : "none" }}>
<div style={{
position: "absolute", top: "50%", left: "50%",
transform: "translate(-50%, -50%)",
fontFamily: "'Cormorant Garamond', Georgia, serif",
fontSize: 220, fontWeight: 700, color: "white",
lineHeight: 1, userSelect: "none", letterSpacing: "-0.02em",
opacity: phase === "idle" ? 0 : phase === "dissolve" ? 0 : 1,
filter: phase === "dissolve" ? "blur(4px)" : "blur(0px)",
textShadow: "none",
transition: phase === "show" ? "opacity 0.4s ease" : phase === "dissolve" ? "opacity 0.9s ease 0.1s, filter 0.8s ease" : "none",
}}>A</div>
<SandStorm active={stormActive} onDone={handleStormDone} />
</div>
);
}
const s = {
...
aiPanel: { background: SURFACE, borderRadius: 2, border: `1px solid ${GOLD}33`, marginBottom: 20, overflow: "hidden" },
aiHeader: { background: `linear-gradient(90deg, ${GOLD_DIM}22, transparent)`, padding: "12px 16px", display: "flex", alignItems: "center", gap: 10, borderBottom: `1px solid ${GOLD_DIM}22` },
aiDot: { width: 8, height: 8, borderRadius: "50%", background: GOLD },
aiTitle: { fontSize: 11, letterSpacing: "0.25em", color: GOLD, textTransform: "uppercase" },
aiBody: { padding: "14px 16px" },
aiText: { fontSize: 13, color: TEXT_DIM, lineHeight: 1.7, marginBottom: 14 },
aiRoute: { background: SURFACE2, border: `1px solid ${GOLD_DIM}22`, borderRadius: 2, padding: "10px 14px", display: "flex", alignItems: "center", gap: 10, cursor: "pointer", marginBottom: 8 },
...
if (!splashDone) return <SplashScreen onComplete={() => { setSplashDone(true); setTimeout(() => setContentVisible(true), 100); }} />;

if (checking) return <div style={{ background: BLACK, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", color: GOLD, fontSize: 22, letterSpacing: "0.2em" }}>AURUM</div>;

return (
<div style={{ ...s.app, opacity: contentVisible ? 1 : 0, transition: "opacity 0.7s ease" }}>
{!user ? (
<AuthView onAuth={() => {}} />
) : (
<>
